...
class FuelInternalReceiving(models.Model):
    _name = "ship.fuel.internal.receiving"
    _description = "Kế hoạch nhận nhiên liệu nội địa"
    ...
    fuel_condition_ids = fields.One2many(
        "ship.fuel.oil.tank.condition",
        "fuel_internal_receiving_id",
    ...
    def _get_bunkering_safety_checklist_html(self):
        ...
        variable_name = (
            CONST_UTILITIES.HTML_SHIP_FUEL_INTERNAL_RECEIVING_BUNKERING_SAFETY_CHECKLIST
        )
    ...
    def create(self, vals_list):
        for vals in vals_list:
            model_name = "ship.fuel.internal.receiving"
            vals["ref"] = self.env["ir.sequence"].next_by_code(model_name)
        result = super(FuelInternalReceiving, self).create(vals_list)
    ...
    def write(self, vals):
        self.ensure_one()
        result = super(FuelInternalReceiving, self).write(vals)
    ...
    def _compute_fuel_received(self):
        for record in self:
            # Find the latest ship.fuel.internal.calculator record
            latest_calculator = self.env["ship.fuel.internal.calculator"].search(
    ...
    def _create_default_fuel_tank_conditions(self):
        # Create one before FuelOilTankCondition
        self.env["ship.fuel.oil.tank.condition"].create(
            {"fuel_internal_receiving_id": self.id, "condition": "before"}
        )

        # Create one after FuelOilTankCondition
        self.env["ship.fuel.oil.tank.condition"].create(
            {"fuel_internal_receiving_id": self.id, "condition": "after"}
        )

    def _get_default_fuel_checks(self):
        default_value_model = self._get_default_value_model()
        variable_name = (
            CONST_UTILITIES.RELATION_SHIP_FUEL_INTERNAL_RECEIVING_FUEL_CHECKS
        )
        ...
        if default_checks:
            fuel_check_ids = self.env["ship.fuel.check"].create(
                [
                    {
                        "name": check.text_1,
...
class FuelOilTankCondition(models.Model):
    _name = "ship.fuel.oil.tank.condition"
    _description = "Fuel Oil Tank Condition"

    fuel_internal_receiving_id = fields.Many2one(
        "ship.fuel.internal.receiving", string="Fuel Internal Receiving"
    )
    tank_datetime = fields.Datetime(string="Tank Datetime", default=fields.Datetime.now)
    condition = fields.Selection(
        [("before", "Before"), ("after", "After")], string="Condition"
    )
    ...
    def _compute_last_measured_weights(self):
        for record in self:
            fuel_internal_receiving_id = record.fuel_internal_receiving_id
            if record.condition == "before":
                ...
                previous_condition = self.env["ship.fuel.oil.tank.condition"].search(
                    [
                        ("condition", "=", "before"),
                        (
                            "fuel_internal_receiving_id",
                            "=",
                            fuel_internal_receiving_id.id,
                        ),
                    ],
                    limit=1,
                )
    ...
    @api.depends("last_measured_weight_DO")
        ...
        for record in self:
            if record.condition == "after":
                record.consumption_between_measurements_DO = (
                    ...
                    + record.fuel_internal_receiving_id.do_received
                )
            else:
                record.consumption_between_measurements_DO = (
                    record.last_measured_weight_DO - record.total_DO_weight
    ...
    @api.depends("last_measured_weight_FO")
        ...
        for record in self:
            if record.condition == "after":
                record.consumption_between_measurements_FO = (
                    ...
                    + record.fuel_internal_receiving_id.hfo_received
                )
            else:
                record.consumption_between_measurements_FO = (
                    record.last_measured_weight_FO - record.total_FO_weight
...
class FuelCheck(models.Model):
    _name = "ship.fuel.check"
    _description = "Fuel Checks during Fuel Receiving"
...
class FuelTankMeasurement(models.Model):
    _name = "ship.fuel.tank.measurement"
    _description = "Fuel Tank Measurement"
    ...
    percentage_fill = fields.Float(
        string="Percentage filling", compute="_compute_percentage_fill"
    )
    fuel_internal_receiving_id = fields.Many2one(
        "ship.fuel.internal.receiving",
        string="Fuel Internal Receiving",
    ...
    @api.depends("tank_name", "fuel_internal_receiving_id")
    def _compute_measure_type(self):
        for record in self:
            if record.tank_name:
                fuel_internal_receiving_id = record.fuel_internal_receiving_id
                if fuel_internal_receiving_id:
                    ...
                    if fuel_condition_ids:
                        for fuel_condition_id in fuel_condition_ids:
                            fuel_tank = self.env["ship.fuel.tank"].search(
                                [
...
class FuelTank(models.Model):
    _name = "ship.fuel.tank"
    _description = "Fuel Tank"

    fuel_oil_tank_condition_id = fields.Many2one(
        "ship.fuel.oil.tank.condition", string="Fuel Oil Tank Condition"
    )
    ...
    @api.depends("measurement", "fuel_oil_tank_condition_id")
    def _compute_calculated_volume(self):
        for record in self:
            ...
            if record.measurement >= 0 and record.fuel_oil_tank_condition_id:
                # Calculate volume based on sounding table values
                fuel_oil_tank_condition = record.fuel_oil_tank_condition_id
    ...
    @api.depends("volumetric_at_150C", "fuel_oil_tank_condition_id")
    def _compute_weight(self):
        for record in self:
            if record.volumetric_at_150C >= 0 and record.fuel_type:
                if (
                    record.fuel_type == "hfo"
                    and record.fuel_oil_tank_condition_id
                    and record.fuel_oil_tank_condition_id.density_hfo
                ):
                    record.weight = (
                        record.volumetric_at_150C
                        * record.fuel_oil_tank_condition_id.density_hfo
                    )
                elif (
                    record.fuel_type == "do"
                    and record.fuel_oil_tank_condition_id
                    and record.fuel_oil_tank_condition_id.density_do
                ):
                    record.weight = (
                        record.volumetric_at_150C
                        * record.fuel_oil_tank_condition_id.density_do
                    )
                else:
                    record.weight = 0.0
            else:
...
class SoundingBook(models.Model):
    _name = "ship.sounding.book"
    _description = "Sounding Book"

    table_type = fields.Selection(
        [("trim", "Trim measurement"), ("heel", "Heel measurement")],
        default="trim",
        string="Table type",
    )
    TANK_NAME_SELECTION = [
    ...
    sounding_table_ids = fields.One2many(
        "ship.sounding.table", "sounding_book_id", string="Sounding Tables"
    )
    company_id = fields.Many2one(
        "res.company", required=True, default=lambda self: self.env.company
    )
...
class SoundingTable(models.Model):
    _name = "ship.sounding.table"
    _description = "Sounding Table"
    company_id = fields.Many2one(
        "res.company", required=True, default=lambda self: self.env.company
    )

    # Define fields to store sound measure to volume data
    table_value = fields.Float("Value of trim/heel")
    sound = fields.Float("Sound (m)")
    ullage = fields.Float("Ullage (m)")
    volume = fields.Float("Volume (m3)")
    sounding_book_id = fields.Many2one("ship.sounding.book", string="Sounding Book")


class FuelOilSampleRecord(models.Model):
    _name = "ship.fuel.internal.oil.sample.record"
    ...
    # Many2one field to link to ship.fuel.internal.receiving
    fuel_internal_receiving_id = fields.Many2one(
        "ship.fuel.internal.receiving", string="Fuel Internal Receiving"
    )
...
class Bunker(models.Model):
    _name = "ship.fuel.internal.bunker.barge"
