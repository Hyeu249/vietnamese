...
class JobQuoteFinalParameterSet(models.Model):
    _name = "docking.job.quote.final.parameter.set"
    _description = "Job Quote final parameter set records"
    ...
    approval_status = fields.Selection(
        ...
        related="job_quote_id.cost_settlement_report_id.approval_status",
        readonly=True,
        tracking=True,
    )
    ...
    def create(self, vals_list):
        for vals in vals_list:
            model_name = "docking.job.quote.final.parameter.set"
            vals["ref"] = self.env["ir.sequence"].next_by_code(model_name)
        result = super(JobQuoteFinalParameterSet, self).create(vals_list)

        for record in result:
            record._get_value_from_expected_parameter_set()
    ...
    def write(self, vals):
        ...
        result = super(JobQuoteFinalParameterSet, self).write(vals)
    ...
    def _get_value_from_expected_parameter_set(self):
        self.ensure_one()

        job_quote_id = self.job_quote_id
        if job_quote_id:
            expected_id = job_quote_id._get_job_quote_expected_parameter_set_id()
            if expected_id:
                self.quantity = expected_id.quantity
                self.weight = expected_id.weight
                self.length = expected_id.length
                self.width = expected_id.width
                self.height = expected_id.height
                self.factor = expected_id.factor
                self.factor_note_option_id = expected_id.factor_note_option_id
