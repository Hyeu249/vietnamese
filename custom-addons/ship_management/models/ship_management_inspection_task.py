...
class ShipManagementInspectionTask(models.Model):
    _name = "ship.ship.management.inspection.task"
    _description = "Ship management inspection task records"
    ...
    task_name = fields.Char(
        "Task name",
        related="ship_management_inspection_task_metadata_id.name",
        tracking=True,
    )
    task_description = fields.Char(
        "Task description",
        related="ship_management_inspection_task_metadata_id.description",
        tracking=True,
    )
    ...
    ship_management_inspection_task_metadata_id = fields.Many2one(
    ...
    ship_management_inspection_scope_id = fields.Many2one(
    ...
    inspection_plan_id = fields.Many2one(
        ...
        related="ship_management_inspection_scope_id.inspection_plan_id",
    ...
    _sql_constraints = [
        (
            "unique_ship_management_inspection_scope_id_ship_management_inspection_task_metadata_id",
            "unique (ship_management_inspection_scope_id, ship_management_inspection_task_metadata_id)",
            "ship_management_inspection_scope_id and ship_management_inspection_task_metadata_id must be unique.",
        ),
    ]
    ...
    def create(self, vals_list):
        for vals in vals_list:
            model_name = "ship.ship.management.inspection.task"
            vals["ref"] = self.env["ir.sequence"].next_by_code(model_name)

        result = super(ShipManagementInspectionTask, self).create(vals_list)
        return result
    ...
    def write(self, vals):
        ...
        result = super(ShipManagementInspectionTask, self).write(vals)
    ...
    def _send_noti_to_people_need_to_fix_the_task(self):
        ...
        for group_xml_id in group_xml_ids:
            ...
            subject = f"Chưa thỏa mãn nội dung kiểm tra của P.QLT"
    ...
    def _send_noti_to_technical_expert_to_confirm_the_task(self):
        ...
        classes = "title_inspection_plan_color"
        subject = f"Cần xác nhận sửa chữa từ nội dung kiểm tra của P.QLT"
