...
class IrModelFields(models.Model):
    ...
    is_based_on_and_of = fields.Boolean(
        "Is based on and of", compute="get_is_based_on_and_of"
    )

    @api.depends("default_one2many")
    def get_is_based_on_and_of(self):
        for record in self:
            if record.default_one2many and "based_on_and_of" in record.default_one2many:
                record.is_based_on_and_of = True
            else:
                record.is_based_on_and_of = False
